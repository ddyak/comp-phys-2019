...
/* equation of the form f(e)=0, where e is Zero-point energy
...
static const auto df = [](float E) {
        float xi = -E/U0;
        return a*a*U0*std::pow(std::sin(std::sqrt(2*a*a*U0*(1-xi))), 2)/std::sqrt(2*a*a*U0*(1-xi)) + 1.f/(xi*xi*2*std::sqrt(1.f/xi - 1));
};
...
int main() {
    ...
    std::cout << "FixedPointIteration method: " << FixedPointIteration(err, f, E0) << std::endl;
