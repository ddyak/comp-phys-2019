...
RungeKuttaMethods(int N, double alpha, std::tuple<double, double> InitPoint, std::pair<double, double> bounds)
{
    ...
    for (int i = 0; i < N; ++i) 
    {
        x.push_back(x[i] + h * ((1 - alpha) * f1(x[i], y[i], y[i]) + alpha * f1(x[i] + h * 0.5 / alpha * f1(x[i], y[i], t[i]),
                                                                                y[i] + h * 0.5 / alpha * f2(x[i], y[i], t[i]),
                                                                                t[i] + h * 0.5 / alpha)));
        y.push_back(y[i] + h * ((1 - alpha) * f2(x[i], y[i], y[i]) + alpha * f2(x[i] + h * 0.5 / alpha * f1(x[i], y[i], t[i]),
                                                                                y[i] + h * 0.5 / alpha * f2(x[i], y[i], t[i]),
                                                                                t[i] + h * 0.5 / alpha)));
        t.push_back(t[i] + h);
    }
...
int main() {
    auto solution = RungeKuttaMethods(iter, alpha, {predator, prey}, {t1, t2});
    for (int i = 0; i < iter; ++i)
    std::cout << std::get<0>(solution)[i] << '\t' << std::get<1>(solution)[i]  << '\t' << std::get<2>(solution)[i] << std::endl;
    return 0; 
}
